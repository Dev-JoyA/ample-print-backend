...
export async function createBankAccount(data, createdBy) {
    ...
    try {
        ...
        if (shouldBeActive) {
            await BankAccount.updateMany({ isActive: true }, { $set: { isActive: false } }, { session });
        }
...
export async function setActiveBankAccount(bankAccountId) {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        ...
        await BankAccount.updateMany({ _id: { $ne: target._id }, isActive: true }, { $set: { isActive: false } }, { session });
    ...
    try {
        ...
        if (wasActive) {
            const next = await BankAccount.findOne({})
                ...
                .session(session);
            if (next) {
                await BankAccount.updateMany({ _id: { $ne: next._id }, isActive: true }, { $set: { isActive: false } }, { session });
                next.isActive = true;
