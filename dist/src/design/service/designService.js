...
const validatePaymentForDesign = async (orderId, session) => {
...
const areAllProductsApproved = async (orderId, session) => {
    const order = await Order.findById(orderId).session(session || null);
    if (!order)
        return false;
...
const hasAnyDesigns = async (orderId, session) => {
...
export const updateDesign = async (id, data, io) => {
    ...
    try {
        ...
        const updatedDesign = await Design.findByIdAndUpdate(id, { ...data, updatedAt: new Date() }, { new: true, runValidators: true, session });
        if (!updatedDesign) {
            await session.abortTransaction();
            session.endSession();
        ...
        await emailService
            .sendDesignReady(user.email, profile.firstName, orderNumber, productName, `${process.env.FRONTEND_URL}/orders/${orderNumber}/design`)
