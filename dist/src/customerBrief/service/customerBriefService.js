import { CustomerBrief, CustomerBriefRole, CustomerBriefStatus, } from "../model/customerBrief.js";
...
export const createOrUpdateCustomerBrief = async (brief, userId, userRole, io) => {
    ...
    if (userRole === UserRole.Customer) {
        ...
        if (order.status !== OrderStatus.Pending &&
            ...
            throw new Error("Customer brief can only be created for orders that are Pending or Order Received or Files Uploaded");
        }
...
export const markBriefAsViewed = async (briefId, userId, userRole, io) => {
    ...
    if (userRole === UserRole.Customer) {
        if (brief.role !== CustomerBriefRole.Admin &&
...
export const markBriefAsViewedByAdmin = async (briefId, adminId, userRole, io) => {
...
export const customerReplyToAdmin = async (brief, userId, userRole, io) => {
    const order = await Order.findById(brief.orderId).exec();
    if (!order) {
        throw new Error("Order not found");
    }
...
export const getCustomerPendingBriefResponses = async (userId) => {
    ...
    const briefs = await CustomerBrief.find({
        ...
        .sort({ createdAt: -1 })
        .lean();
    const pendingResponses = briefs.map((brief) => {
        const order = brief.orderId;
        const product = brief.productId;
        return {
            briefId: brief._id,
...
export const getAdminCustomerBriefs = async (adminId, filters = {}) => {
    const { status, hasFiles, search, page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", } = filters;
    const query = {};
    if (hasFiles) {
        query.$or = [
    ...
    for (const brief of allBriefs) {
        const hasFilesAttached = !!(brief.image ||
            brief.voiceNote ||
            brief.video ||
            brief.logo);
        const order = brief.orderId;
...
export const getOrderBriefStatus = async (orderId) => {
    ...
    const allProductsComplete = productStatus.every((p) => p.status === "complete" || p.status === "no-brief");
    return {
        orderId,
        orderNumber: order.orderNumber,
...
export const markBriefAsComplete = async (briefId, userId, userRole, io) => {
    if (!Types.ObjectId.isValid(briefId)) {
        throw new Error("Invalid brief ID format");
    }
    const brief = await CustomerBrief.findById(briefId);
...
const checkAndUpdateOrderStatus = async (orderId, io) => {
    const order = await Order.findById(orderId);
    if (!order) {
        throw new Error("Order not found");
    }
    if (order.status !== OrderStatus.FilesUploaded) {
        console.log(`Order ${order.orderNumber} is not in FilesUploaded status (current: ${order.status})`);
        return false;
    }
    if (order.invoiceId) {
    ...
    if (allProductsHaveCompleteBrief) {
        console.log(`All briefs for order ${order.orderNumber} are complete. Moving to AwaitingInvoice`);
        order.status = OrderStatus.AwaitingInvoice;
        await order.save();
        if (io) {
            io.to("superadmin-room").emit("order-ready-for-invoice", {
        ...
        console.log(`⏳ Order ${order.orderNumber} not ready - some briefs still pending`);
    }
