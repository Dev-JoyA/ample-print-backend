...
import { Shipping, ShippingMethod, ShippingStatus, } from "../model/shippingModel.js";
...
export const createShipping = async (orderId, data, adminId, io) => {
    ...
    try {
        ...
        if (order.status !== OrderStatus.ReadyForShipping &&
            ...
            throw new Error(`Order must be in ReadyForShipping or Completed status to create shipping. Current status: ${order.status}`);
        }
...
export const updateShippingTracking = async (shippingId, data, adminId, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
...
export const updateShippingStatus = async (shippingId, status, adminId, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const shipping = await Shipping.findById(shippingId).session(session);
        ...
        shipping.trackingHistory.push({
            status,
            location: status === ShippingStatus.Delivered ? "Destination" : "In transit",
...
export const markShippingAsPaid = async (shippingId, invoiceId) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const shipping = await Shipping.findById(shippingId).session(session);
...
export const getAllShipping = async (page = 1, limit = 10) => {
...
export const filterShipping = async (filters) => {
    const { status, method, orderId, userId, isPaid, startDate, endDate, page = 1, limit = 10, } = filters;
    const query = {};
    if (status)
        query.status = status;
    if (method)
...
export const getPendingShipping = async () => {
    return Shipping.find({
        status: ShippingStatus.Pending,
    })
        .populate("orderId", "orderNumber userId")
