...
export const createOrder = async (userId, data, io) => {
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (item.quantity < product.minOrder) {
                throw new Error(`${product.name} minimum order quantity is ${product.minOrder}`);
            }
            orderItems.push({
...
export const superAdminCreateOrder = async (customerId, data, superAdminId, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
...
export const updateOrder = async (orderId, data, userId, userRole) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
...
export const deleteOrder = async (orderId, userId, userRole) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const order = await Order.findById(orderId).session(session);
...
export const getUserOrders = async (userId, page = 1, limit = 10, search, status) => {
...
export const updateOrderStatus = async (orderId, newStatus, userId, userRole, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        ...
        if (newStatus === OrderStatus.AwaitingFinalPayment) {
            ...
            if (user && profile) {
                const activeBank = await BankAccount.findOne({ isActive: true })
                    ...
                    .exec();
                await emailService
                    .sendFinalPaymentReminder(user.email, profile.firstName, order.orderNumber, order.remainingBalance, activeBank
                    ? {
                    ...
                    : undefined)
                    .catch((err) => console.error("Error sending final payment reminder email:", err));
            }
        ...
        if (newStatus === OrderStatus.Completed) {
            ...
            if (user && profile) {
                await emailService
                    .sendShippingSelectionReminder(user.email, profile.firstName, order.orderNumber, `${process.env.FRONTEND_URL}/orders/${order.orderNumber}/shipping`)
                    .catch((err) => console.error("Error sending shipping selection email:", err));
            }
        ...
        if (newStatus === OrderStatus.Shipped) {
            ...
            if (user && profile) {
                await emailService
                    .sendOrderShipped(user.email, profile.firstName, order.orderNumber)
                    .catch((err) => console.error("Error sending order shipped email:", err));
            }
        }
        ...
        if (newStatus === OrderStatus.Cancelled) {
            if (user && profile) {
                await emailService
                    .sendOrderCancelled(user.email, profile.firstName, order.orderNumber)
                    .catch((err) => console.error("Error sending order cancelled email:", err));
            }
        }
...
export const getAllOrders = async (userRole, page = 1, limit = 10, filters) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
...
export const searchByOrderNumber = async (orderNumber, userId, userRole) => {
...
export const filterOrders = async (filters, userRole) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
        throw new Error("Unauthorized");
    }
    const query = {};
...
export const getPaidOrders = async (userRole, page = 1, limit = 10) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
        throw new Error("Unauthorized");
    }
    const skip = (page - 1) * limit;
...
export const getPartiallyPaidOrders = async (userRole, page = 1, limit = 10) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
        throw new Error("Unauthorized");
    }
    const skip = (page - 1) * limit;
...
export const getPendingPaymentOrders = async (userRole, page = 1, limit = 10) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
        throw new Error("Unauthorized");
    }
    const skip = (page - 1) * limit;
...
export const getOrdersReadyForShipping = async (userRole, page = 1, limit = 10) => {
    if (userRole !== UserRole.Admin && userRole !== UserRole.SuperAdmin) {
        throw new Error("Unauthorized");
    }
    const skip = (page - 1) * limit;
...
export const linkInvoiceToOrder = async (orderId, invoiceId, paymentType, depositAmount) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const order = await Order.findById(orderId).session(session);
...
export const addItemToOrderService = async (orderId, userId, productId, quantity) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
...
export const getUserActiveOrders = async (userId, statuses = [
    OrderStatus.OrderReceived,
    OrderStatus.Pending,
    OrderStatus.FilesUploaded,
    OrderStatus.AwaitingInvoice,
