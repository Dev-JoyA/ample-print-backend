...
export const createOrder = async (id, data, io) => {
    ...
    emails(user.email, `New Order created ${order.orderNumber} `, "You have created a new Order", profile.firstName, `Hello ${profile.firstName},
            Your order with ORDER NUMBER :  **${order.orderNumber}** have been created.
            Order Number: ${order.orderNumber}
            Please log in to your dashboard to track your order and expect a follow up email for your invoice `).catch((err) => console.error("Error sending order confirmation email", err));
    return order;
};
...
export const getUserOrder = async (userId, page = 1, limit = 10) => {
...
export const searchByOrderNumber = async (orderNumber) => {
