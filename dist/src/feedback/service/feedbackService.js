...
import { Feedback, FeedBackStatus } from "../model/feedback.js";
...
export const createCustomerFeedback = async (data, userId, io) => {
    ...
    try {
        ...
        const notificationData = {
            ...
            ...(data.designId && {
                ...
                productName: feedback.designId?.productId?.name || "Unknown Product"
            }),
            message: data.message.substring(0, 50) + (data.message.length > 50 ? "..." : ""),
            customerName: feedback.userId?.fullname || profile?.firstName || "Customer",
            priority: "high",
            timestamp: new Date(),
        };
        ...
        try {
            await notificationService.createForAdmins({
                ...
                message: `Customer ${profile?.firstName || 'Customer'} submitted feedback for order #${feedback.orderId.orderNumber}`,
                data: {
                    feedbackId: feedback._id,
                    orderId: feedback.orderId,
                    orderNumber: feedback.orderId.orderNumber,
                    designId: data.designId,
                    productName: feedback.designId?.productId?.name,
...
export const adminRespondToFeedback = async (feedbackId, response, attachments, adminId, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        ...
        if (attachments && attachments.length > 0) {
            feedback.adminAttachments = attachments;
        }
...
export const updateFeedbackStatus = async (feedbackId, status, userId, userRole, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const feedback = await Feedback.findById(feedbackId).session(session);
...
export const filterFeedback = async (filters) => {
...
export const getPendingFeedback = async (page = 1, limit = 10) => {
    const skip = (page - 1) * limit;
    const [feedback, total] = await Promise.all([
...
export const getFeedbackById = async (feedbackId, userId, userRole) => {
...
export const getFeedbackByOrderId = async (orderId, userId, userRole) => {
...
export const getUserFeedback = async (userId, page = 1, limit = 10, status) => {
    const skip = (page - 1) * limit;
...
export const deleteFeedback = async (feedbackId, userId, userRole, io) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const feedback = await Feedback.findById(feedbackId).session(session);
