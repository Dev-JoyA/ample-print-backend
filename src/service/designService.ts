...
export const uploadDesign = async (id: string, data: IDesign, io: Server): Promise<IDesign> => {
    ...
    ).catch(err => console.error("Error sending email to notify customer of design upload", err));
...
export const updateDesign = async (id: string, data: Partial<IDesign>, io: Server): Promise<IDesign> => {
    if (!data.productId) {
    ...
    const profile = await Profile.findOne({userId : user._id}).exec();
    const product = await Product.findOne({_id : updatedDesign.productId}).exec();
...
export const getDesignsByOrderId = async (orderId: string): Promise<IDesign[]> => {
...
export const filterDesigns = async (filters: IDesignFilter) => {
    ...
    if (filters.minVersion !== undefined || filters.maxVersion !== undefined) {
        query.version = {};
        if (filters.minVersion !== undefined) query.version.$gte = filters.minVersion;
        if (filters.maxVersion !== undefined) query.version.$lte = filters.maxVersion;
    }
...
export const getDesignByProductId = async (productId: string): Promise<IDesign[]> => {
