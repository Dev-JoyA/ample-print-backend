...
export const updatedDesignController = async(req: Request, res: Response) => {
  try{
    ...
    const updatedData: Partial<IDesign> = {...req.body,
    uploadedBy : admin._id
    }
    if (files && files.length > 0) {
...
export const getDesignByorderIdController = async(req: Request, res: Response) => {
  try{
    const {id} = req.params;
...
export const getDesignByProductIdController = async(req: Request, res: Response) => {
  try{
    const {id} = req.params;
