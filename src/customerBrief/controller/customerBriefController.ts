...
export const submitCustomerBrief = async (req: Request, res: Response) => {
  try {
    ...
    if (files?.image) briefData.image = getFileUrl(files.image[0]);
    if (files?.voiceNote) briefData.voiceNote = getFileUrl(files.voiceNote[0]);
    if (files?.video) briefData.video = getFileUrl(files.video[0]);
    if (files?.logo) briefData.logo = getFileUrl(files.logo[0]);
