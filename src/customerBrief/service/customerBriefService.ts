import { ICustomerBrief, CreateCustomerBriefDTO, CustomerBrief, CustomerBriefRole } from "../model/customerBrief.js";
...
export const createOrUpdateCustomerBrief = async (
  ...
  const hasContent = brief.description || brief.image || brief.voiceNote || brief.video || brief.logo;
  if (!hasContent) {
  ...
  if (userRole === UserRole.Customer) {
    ...
    if (order.status !== OrderStatus.Pending && order.status !== OrderStatus.OrderReceived) {
      throw new Error("Customer brief can only be created for orders that are Pending or Order Received");
    }
    ...
    if (!customerBrief) {
      throw new Error("Customer must submit a brief first before admin can respond");
    }
...
export const deleteCustomerBrief = async (
  ...
  if (userRole === UserRole.Customer) {
    ...
    const allowedStatuses = [OrderStatus.Pending, OrderStatus.OrderReceived, OrderStatus.FilesUploaded];
    if (!allowedStatuses.includes(order.status)) {
      throw new Error(`Cannot delete brief when order is in ${order.status} status`);
    }
  } else if (userRole === UserRole.SuperAdmin) {
    if (brief.role !== CustomerBriefRole.SuperAdmin) {
      throw new Error("Only super-admin briefs can be deleted by super-admin");
    }
  } else if (userRole !== UserRole.SuperAdmin) {
    throw new Error("Unauthorized to delete this brief");
  }
...
export const getCustomerBriefByOrderId = async (
  ...
  const briefs = await CustomerBrief.find({ orderId: new Types.ObjectId(orderId) })
    .populate("productId", "name price mainImage")
    .populate("designId", "designUrl filename status")
    .sort({ createdAt: 1 })
    .exec();
...
export const getAdminCustomerBriefs = async (
  ...
  const orderIdsWithAdminResponse = adminBriefs.map(ab => ab.orderId.toString());

  let finalOrderIds = orderIdsWithCustomerBrief.map(id => id.toString());
  
  if (hasResponded !== undefined) {
    if (hasResponded) {
      finalOrderIds = finalOrderIds.filter(id => orderIdsWithAdminResponse.includes(id));
    } else {
      finalOrderIds = finalOrderIds.filter(id => !orderIdsWithAdminResponse.includes(id));
    }
  }
  ...
  const briefsWithStatus = briefs.map(brief => {
    const hasAdminResponse = orderIdsWithAdminResponse.includes(brief.orderId.toString());
    return {
      ...brief.toObject(),
